...
std::uniform_real_distribution<float> Distribution_01(-1.0f, 1.0f);
...
void handleElasticCollision(Ball& ball1, Ball& ball2) {
    ...
    if (relativeSpeed < 0) {
        ...
        float impulse = (1.0f + dampingFactor) * relativeSpeed / (1.0f / ball1.radius + 1.0f / ball2.radius);
		...
		ball1.pos.x -= overlap * nx / 2.0f;
		ball1.pos.y -= overlap * ny / 2.0f;
		ball2.pos.x += overlap * nx / 2.0f;
		ball2.pos.y += overlap * ny / 2.0f;


    }
}

const int maxnumBalls = 400;
...
void resolveCollisions(int i) {
	...
	if(abs(p.pos.y) > halfboundsize.y){
		p.pos.y = 0.99*halfboundsize.y * sgn(p.pos.y);
		p.speed.y *= -1 * damping;
	}

	// Check bounding box in x
	if(abs(p.pos.x) > (halfboundsize.x)){
		p.pos.x = 0.99*(halfboundsize.x) * sgn(p.pos.x);
		p.speed.x *= -1 * damping;
	}
...
float pressureMutiplier = .1f;

float SmoothingFunc(float dist, float radius) {
	if (dist >= radius) return 0;
	
	float volume = glm::pi<float>() * pow(radius, 4)/6;
...
float CalculateDensity(glm::vec3 samplePoint) {
	...
	for(int i=0; i<maxnumBalls; i++){
		Ball& p = BallContainer[i];
		float dist = glm::length(p.pos - samplePoint);
...
glm::vec3 GetRandomDir(){
	glm::vec3 rar = glm::vec3(Distribution_01(gen), Distribution_01(gen), Distribution_01(gen));
	return rar;
}
...
glm::vec3 CalculatePressureForce(int particleIndex){
	...
	Ball& p1 = BallContainer[particleIndex];

	for(int otherParticleIndex = 0; otherParticleIndex < maxnumBalls; otherParticleIndex++)
	{
		if (particleIndex == otherParticleIndex) continue;
		Ball& p2 = BallContainer[otherParticleIndex];
		//////////////////////////////////////////////
		glm::vec3 offset = p2.pos - p1.pos;
		float dist = glm::length(offset);
		glm::vec3 dir = dist == 0 ? GetRandomDir() : offset / dist;
		float slope = SmoothingFunc(dist, smoothingRadius);
		float density = p2.density;
		float sharedPressure = CalculateSharedPressure(density, p1.density);
	...
	do {
		...
		// Simulate all particles
			for(int i=0; i<maxnumBalls; i++){
				...
				p.predictedPos = p.pos * p.speed * (float)delta; 
			}

			for(int i=0; i<maxnumBalls; i++){
				Ball& p = BallContainer[i];
				p.density = CalculateDensity(p.predictedPos);
			} 

			for(int i=0; i<maxnumBalls; i++){
				Ball& p = BallContainer[i];
				glm::vec3 pressureForce = CalculatePressureForce(i);
				glm::vec3 pressureAcceleration = pressureForce / p.density;
				p.speed += pressureAcceleration * (float)delta;
		...
		/// Fill the buffers with the final result
		for(int i=0; i<maxnumBalls; i++){
			Ball& p = BallContainer[i];
			// Fill the GPU buffer
