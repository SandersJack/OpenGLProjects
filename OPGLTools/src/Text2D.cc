...
Text2D *Text2D::fInstance = nullptr;

Text2D *Text2D::GetInstance() {
    if(!fInstance)
        fInstance = new Text2D();
    return fInstance;
}
...
void Text2D::initFreeType(const char* fontPath, int fontSize) {
    // FreeType
    // --------
    FT_Library ft;
